...
for record_file in sorted(log_dir.iterdir()):
    ...
    record_df = (
        pd.read_json(record_file, orient="records", convert_dates=False)
        .query(f"fairy_id!=0 and dev_time>{begin_ts} and id>{last_id}")
        .sort_values(by="id")
        .set_index("id", drop=False)
    )
    record_df["fairy_name"] = record_df["fairy_id"].apply(
        lambda fairy_id: gamedata["fairy"][fairy_id]["name"]
    )
    if len(record_df) == 0:
        continue
...
for record_file in sorted(log_dir.iterdir()):
    ...
    record_df = (
        pd.read_json(record_file, orient="records", convert_dates=False)
        .query(f"quality_lv==0 and dev_time>{begin_ts} and id>{last_id}")
        .sort_values(by="id")
        .set_index("id", drop=False)
    )
    record_df["equip_rank"] = record_df["equip_id"].apply(
        lambda equip_id: gamedata["equip"][equip_id]["rank"]
    )
    record_df["equip_name"] = record_df["equip_id"].apply(
        lambda equip_id: gamedata["equip"][equip_id]["name"]
    )
    if len(record_df) == 0:
        continue
    last_id = record_df["id"].max()
...
for record_file in sorted(log_dir.iterdir()):
    ...
    record_df["gun_rank"] = record_df["gun_id"].apply(
        lambda gun_id: gamedata["gun"][gun_id]["rank"]
    )
    record_df["gun_name"] = record_df["gun_id"].apply(
        lambda gun_id: gamedata["gun"][gun_id]["name"]
    )
    if len(record_df) == 0:
        continue
    last_id = record_df["id"].max()
